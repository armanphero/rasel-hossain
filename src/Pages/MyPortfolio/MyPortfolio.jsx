...
import SingleProf from '../SingleProf/SingleProf';

const MyPortfolio = ({ setModalImg }) => {
    return (
        <section className="portfolio--section content-container" id="MyPortfolio">
            <div className="portfolio--container-box">
                <div className="portfolio--container">
                    <p className="sub--title">Recent Projects</p>
                    <h2 className="section--heading">Google work</h2>
                </div>
                <div>
            ...
            <div className="google--section--container">
                {data?.google?.map((item, index) => <SingleProf item={item} key={index} setModalImg={setModalImg}></SingleProf>)}
            </div>
            <div className="portfolio--container">
                <h2 className="section--heading">Facebook work</h2>
            </div>
            <div className="portfolio--section--container">
                {data?.portfolio?.map((item, index) => <SingleProf item={item} key={index} setModalImg={setModalImg}></SingleProf>)}
            </div>
        </section>
    );
};
